...
if ($usercheck == "OK"){
    ...
    if (mysqli_num_rows($threaddb) > "0")  {
        $thread = mysqli_fetch_object($threaddb);
        if ($thread->Sender == $userid) {
            mysqli_query($dbcon, "UPDATE UserMessages SET `DeletedSend` = '1' WHERE id = '$threadid'") OR die(mysqli_error($dbcon));
            echo "OK";
        }
        else if ($thread->Receiver == $userid) {
            mysqli_query($dbcon, "UPDATE UserMessages SET `DeletedRec` = '1' WHERE id = '$threadid'") OR die(mysqli_error($dbcon));
            echo "OK";
        }
        mysqli_query($dbcon, "INSERT INTO UserProtocol (`User`, `IP`, `Priority`, `Activity`, `Comment`) VALUES ('$userid', '$user_ip_adress', '1', 'Private Message Deleted', '$threadid')") OR die(mysqli_error($dbcon));
    }
}
