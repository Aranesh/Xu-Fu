...
if ($userid) {
    ...
    if (mysqli_num_rows($userdb) > "0") {
        ...
        if ($user->ComSecret == $comsecret) {
            set_settings($userid,"9",$colaction);
            mysqli_query($dbcon, "INSERT INTO UserProtocol (`User`, `IP`, `Priority`, `Activity`) VALUES ('$user->id', '$user_ip_adress', '1', 'Rematch to include steps setting changed')") OR die(mysqli_error($dbcon));
            echo "OK";
            $result = "done";
        }
    }
