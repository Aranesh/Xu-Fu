...
if ($userid) {
    ...
    if (mysqli_num_rows($userdb) > "0") {
        ...
        if ($user->ComSecret == $comsecret) {
            mysqli_query($dbcon, "UPDATE Users SET PrFavPet = '$new_pet' WHERE id = '$user->id'");
            mysqli_query($dbcon, "INSERT INTO UserProtocol (`User`, `IP`, `Priority`, `Activity`) VALUES ('$user->id', '$user_ip_adress', '1', 'User Profile - Edited Favourite Pet')") OR die(mysqli_error($dbcon));
            echo "OK";
            $result = "done";
        }
    }
